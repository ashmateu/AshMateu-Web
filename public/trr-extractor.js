(function() {
  ...
  const defaultMargin = Math.max(productData.priceTRR * 0.25, 120);
  const suggestedPrice = Math.round(productData.priceTRR + defaultMargin);

  // 5. Crear Modal en Pantalla
  const overlay = document.createElement('div');
  overlay.id = 'ash-trr-extractor-modal';
  overlay.style.cssText = 'position:fixed;inset:0;z-index:9999999;background:rgba(10,10,10,0.8);backdrop-filter:blur(10px);display:flex;align-items:center;justify-content:center;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;color:#0A0A0A;padding:20px;box-sizing:border-box;';
  ...
  modal.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
      ...
      <div style="grid-column:span 2;">
        ...
        <input id="ash-styling-tip" type="text" placeholder="Ej: 'Combínalo con un sastre oversize para la noche.'" style="width:100%;padding:8px 10px;border-radius:8px;border:1px solid #B5A898;background:#fff;font-size:13px;box-sizing:border-box;" />
      </div>
    </div>
  ...
  document.getElementById('ash-submit-btn').onclick = async function() {
    ...
    try {
      const res = await fetch('https://ashmateu.com/api/mercadito/import', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      if (res.ok) {
        alert('✦ ¡Pieza publicada con éxito en El Mercadito de Ash!');
        close();
      } else {
        alert('Hubo un detalle al guardar en la web. Intenta nuevamente.');
        this.disabled = false;
        this.innerText = 'Publicar en El Mercadito ↗';
      }
    } catch (e) {
